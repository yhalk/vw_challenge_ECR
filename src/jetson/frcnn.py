...
class ObjectPredictor:
  #os.environ['CUDA_VISIBLE_DEVICES'] = ''
        ...
        def ret_detected_objects(self,detected_objects):
  	   ...
  	   # apply the spatial pyramid pooling to the proposed regions
           ...
           for key in bboxes:
  			...
  			#cv2.putText(img, textLabel, textOrg, cv2.FONT_HERSHEY_DUPLEX, 0.3, (0, 0, 0), 1)
                        ...
                        angle_between_robot_centre_and_detected_object = self.angle_between((real_x1,(real_y1+real_y2)/2.0), (width/2.0,0))
                        focal_length_mm = 1.0
                        average_real_object_height_mm = 1.0
                        image_height_px = height
                        object_height_px = self.distance([real_x1,real_y1], [real_x1,real_y2])
                        sensor_height_mm = 1.0
                        distance_between_robot_centre_and_detected_object = (focal_length_mm * average_real_object_height_mm * image_height_px) / float(object_height_px * sensor_height_mm)
